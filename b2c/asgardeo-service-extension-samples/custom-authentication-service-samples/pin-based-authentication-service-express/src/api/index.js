...
const config = {
    AUTH_MODE: process.env.AUTH_MODE || "second_factor",
    BASE_WSO2_IAM_PROVIDER_URL: process.env.BASE_WSO2_IAM_PROVIDER_URL || "https://api.asgardeo.io",
    HOST_URL: process.env.HOST_URL || "https://pin-authenticator.vercel.app",
};
...
if (process.env.USER_CONFIG) {
    ...
    console.log("Loaded users from environment");
} else {
    try {
...
app.post("/api/validate-pin", (req, res) => {
    ...
    console.log("User Authenticating: ", userAuthenticating);
    ...
    if (userAuthenticating && userReferencedForPin && userAuthenticating.id === userReferencedForPin.id) {
        ...
        console.log("User Authenticated: ", userReferencedForPin);
    } else {
